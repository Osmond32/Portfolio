...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 right-0 w-full h-20 bg-dark/80 backdrop-blur-md flex items-center justify-end px-8 z-30 border-b border-gray-800/50">
      
      {/* Contenitore principale a destra che raggruppa Pulsante + Bandiere */}
      <div className="flex items-center gap-8"> {/* gap-8 distanzia il bottone dalle bandiere */}
        
        {/* Pulsante CV (dinamico) */}
        <button 
          onClick={handleCvClick}
          className="bg-primary/10 text-primary border border-primary px-5 py-2.5 rounded-lg font-medium hover:bg-primary hover:text-dark transition-all duration-300 shadow-[0_0_10px_rgba(0,229,255,0.2)] hover:shadow-[0_0_15px_rgba(0,229,255,0.6)]"
        >
          {isCvPage ? t('header.downloadCV') : t('header.viewCV')}
        </button>

        {/* Sfondo bianco, bordi e ombra per le bandierine */}
        <div className="flex items-center gap-4 bg-white p-2 px-3 rounded-lg border border-gray-200 shadow-md">
